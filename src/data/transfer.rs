...
impl DataState {
    fn valid_count(&self) -> usize {
        self.chromosomes
            .iter()
            .filter(|c| matches!(c, ChromState::Valid { .. }))
            .count()
    }
...
impl PackFileState {
    ...
    fn expected_size(&self) -> u64 {
        match self {
            ...
            Self::SizeMismatch { expected_size, .. } | Self::Missing { expected_size, .. } => {
                *expected_size
            }
        }
    }
...
fn assess_pack_state(local_base: &Path, manifest: &PackManifest) -> Vec<PackFileState> {
    let mut states: Vec<PackFileState> = manifest
        .files
        .iter()
        .map(|(rel_path, entry)| {
            ...
            if local_path.exists() {
                ...
                if local_size == entry.size {
                    ...
                    PackFileState::SizeMismatch {
                        rel_path: rel_path.clone(),
                        expected_size: entry.size,
                    }
                }
            } else {
                PackFileState::Missing {
                    rel_path: rel_path.clone(),
                    expected_size: entry.size,
                }
            }
...
fn status(output: &dyn Output) -> Result<(), CohortError> {
    ...
    if state.is_complete() {
        output.success(&format!(
            ...
            state.tier,
            state.valid_count(),
            human_size(state.manifest.total_size),
        ));
    } else {
...
fn pull(full: bool, dry_run: bool, yes: bool, output: &dyn Output) -> Result<(), CohortError> {
    ...
    if !yes && total_bytes > 0 && std::io::stdin().is_terminal() && !confirm_download(total_bytes) {
    ...
    for (i, chrom_state) in to_download.iter().enumerate() {
        ...
        match result {
            ...
            Err(e) => {
                output.warn(&format!(
                    "chr{chrom} failed: {e} (partial saved, will resume next run)"
                ));
            }
        }
    }
...
pub fn pull_pack(
    ...
    output.status(&format!(
        "Pack '{}' — {} ({})",
        pack.name, pack.source, pack.size_human
    ));
    ...
    let final_valid = final_states
        .iter()
        .filter(|s| matches!(s, PackFileState::Valid { .. }))
        .count();
...
fn verify(
    ...
    for pack in &packs_to_verify {
        ...
        if checksums && manifest.schema_version >= 2 {
            let files_with_hashes: Vec<_> = manifest
                .files
                .iter()
        ...
        if mismatched.is_empty() && missing.is_empty() && checksum_failures == 0 {
            ...
            output.warn(&format!(
                "  {}: {}/{} valid, {} mismatched, {} missing, {} checksum failures",
                pack.id,
                valid,
                states.len(),
                mismatched.len(),
