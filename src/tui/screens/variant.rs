...
impl VariantScreen {
    ...
    fn draw_table(&self, frame: &mut Frame, area: Rect) {
        let focused = self.sub_mode == SubMode::Browse;
        ...
        let widths = compute_widths(batch, &cols, body_width);
        let mut header_spans: Vec<Span> = Vec::with_capacity(cols.len() + 1);
        ...
        for row in start..end {
            ...
            let glyph = if is_focus && focused { theme::FOCUS_GLYPH } else { " " };
            spans.push(Span::styled(
                format!("{glyph} "),
                Style::default().fg(theme::ACCENT),
            ));
            for (c, w) in cols.iter().zip(widths.iter()) {
                spans.push(Span::styled(pad(&format_cell(batch, *c, row), *w), style));
            }
    ...
    fn status_text(&self) -> String {
        let rg = self.scroller.row_group_count().max(1);
        let cur = self.scroller.current_row_group() + 1;
        format!("{cur}/{rg}")
    }

    fn draw_title_row(&self, frame: &mut Frame, area: Rect) {
        let status = self.status_text();
        let w = area.width as usize;
        let gap = w.saturating_sub(self.title.chars().count() + status.chars().count() + 2);
        let line = Line::from(vec![
            Span::styled(format!(" {}", self.title), Style::default().fg(theme::ACCENT).bold()),
            Span::raw(" ".repeat(gap)),
    ...
    fn draw_filter_bar(&self, frame: &mut Frame, area: Rect) {
        let line = Line::from(vec![
            Span::styled(format!("{} ", theme::FOCUS_GLYPH), Style::default().fg(theme::ACCENT)),
            Span::styled("/", Style::default().fg(theme::WARN).bold()),
            Span::raw(" "),
            Span::styled(self.filter_bar.buf.as_str(), Style::default().fg(theme::FG).bold()),
    ...
    fn draw_carrier_panel(&self, frame: &mut Frame, area: Rect) {
        ...
        let vid = if panel.vid.is_empty() { "<row>" } else { panel.vid.as_str() };
        let mut lines: Vec<Line> = Vec::new();
        lines.push(Line::from(vec![
            Span::styled(format!("{} ", theme::FOCUS_GLYPH), Style::default().fg(theme::ACCENT)),
            Span::styled(vid, Style::default().fg(theme::FG).bold()),
    ...
    fn draw_column_picker(&self, frame: &mut Frame, area: Rect) {
        ...
        let items: Vec<ListItem> = schema
            ...
            .enumerate()
            .map(|(i, f)| {
                let mark = if picker.selected[i] { "●" } else { "·" };
                let style = if i == picker.cursor {
                    Style::default().fg(theme::FG).bold()
                } else {
                    Style::default().fg(theme::MUTED)
                };
                let glyph = if i == picker.cursor { theme::FOCUS_GLYPH } else { " " };
...
impl Screen for VariantScreen {
    ...
    fn draw(&mut self, frame: &mut Frame, area: Rect, _log: &LogTail) {
        ...
        self.draw_title_row(frame, v[0]);
        self.draw_table(frame, v[1]);
        self.draw_detail(frame, v[2]);
        match self.sub_mode {
            SubMode::FilterEdit => self.draw_filter_bar(frame, v[3]),
            SubMode::Carrier => self.draw_carrier_panel(frame, v[3]),
            _ => {}
        }

        let err = self.error.as_deref().or(self.filter_bar.parse_error.as_deref());
        if let Some(e) = err {
            frame.render_widget(
                Paragraph::new(Span::styled(format!(" {e}"), Style::default().fg(theme::BAD))),
                v[4],
            );
        }
        let hint = match self.sub_mode {
            SubMode::FilterEdit => " enter apply  esc cancel",
            SubMode::Carrier => " j k scroll  esc close",
            SubMode::ColumnPicker => " space toggle  enter close",
            SubMode::Browse => " / filter  ! cols  c carriers  { } page  g G  q",
        };
        frame.render_widget(
