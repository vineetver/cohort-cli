...
impl ErrorKind {
    pub fn from_cohort(e: &CohortError) -> Self {
        match e {
            CohortError::Input(_) => Self::Input,
            CohortError::DataMissing(_) => Self::DataMissing,
            CohortError::Resource(_) => Self::Resource,
            CohortError::Analysis(_) => Self::Analysis,
            CohortError::Internal(_) => Self::Internal,
        }
    }
}
...
impl App {
    ...
    fn apply_transition(&mut self, _top_idx: usize, transition: Transition) -> bool {
        ...
        if changed {
            self.prune_error_for_stack();
        }
        changed
    }

    #[allow(dead_code)]
    pub(super) fn set_error_from(&mut self, source: ErrorSource, e: &CohortError) {
        self.error = Some(ErrorMessage {
            kind: ErrorKind::from_cohort(e),
            source,
            detail: e.to_string(),
            screen_depth: self.screens.len(),
        });
    }
    ...
    #[allow(dead_code)]
    pub(super) fn error(&self) -> Option<&ErrorMessage> {
        self.error.as_ref()
    }

    fn prune_error_for_stack(&mut self) {
        let depth = self.screens.len();
        if let Some(e) = &self.error {
            if e.screen_depth > depth {
                self.error = None;
            }
        }
    }
