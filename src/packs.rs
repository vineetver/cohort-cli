...
/// Where a pack's source files live relative to the favor data root.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SourceType {
    /// Tables under {favor_root}/tissue/{table}/
    Tissue,
    /// Tables at {favor_root}/{table}/ (e.g. rollups)
    Root,
}

/// Definition of an add-on data pack.
///
/// This struct is the **single source of truth** for the data pipeline.
/// `favor data publish` reads these definitions to generate manifests and upload.
/// `favor data pull` reads these definitions to resolve local download paths.
/// Nothing else should define pack structure.
pub struct Pack {
    pub id: &'static str,
    pub name: &'static str,
    ...
    /// Remote base URL. Empty = use global default. Community packs can point anywhere.
...
impl Pack {
    ...
    /// The directory to walk when generating manifests (publish).
    /// This is the ONLY place that resolves source → filesystem path.
    ...
    /// The directory where this pack's files land on the client (pull).
    /// This is the ONLY place that resolves pack → local path.
