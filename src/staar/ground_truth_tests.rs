//! Ground truth tests matching the R STAAR package formulas.
//!
//! Test vectors generated by `test_reference/generate_ground_truth.py` which
//! implements the exact R/C++ formulas from xihaoli/STAAR. Any p-value
...
mod tests {
    ...
    // SPA binary uses serde_json::Value because R returns NULL for some
    // fields which serializes as {} — we validate what's available.
    ...
    fn load() -> GroundTruth {
        ...
        // R serializes NULL as {} — replace with null for serde compatibility
    ...
    fn assert_close(name: &str, actual: f64, expected: f64, tol: f64) {
        ...
        let rel = if expected.abs() > 1e-30 { diff / expected.abs() } else { diff };
        assert!(
            diff < tol || rel < tol,
    ...
    fn beta_weights_match_r() {
        ...
        for (i, case) in gt.weights.iter().enumerate() {
            ...
            assert_close(&format!("beta(0.5,0.5)[{i}]"), w05, case.beta_0_5_0_5, TOL_TIGHT);
        }
    }
    ...
    fn staar_continuous_matches_r() {
        ...
        let k_scaled: Vec<Vec<f64>> = s.k.iter().map(|row| {
            row.iter().map(|&v| v * inv_s2).collect()
        }).collect();
        ...
        assert_close("Burden(1,25)", result.burden_1_25, s.expected.burden_1_25, TOL);
        assert_close("Burden(1,1)", result.burden_1_1, s.expected.burden_1_1, TOL);
        assert_close("SKAT(1,25)", result.skat_1_25, s.expected.skat_1_25, TOL_SKAT);
        ...
        assert!(result.acat_v_1_25.is_finite(), "ACAT-V(1,25) should be finite");
        assert!(result.acat_v_1_1.is_finite(), "ACAT-V(1,1) should be finite");
    ...
    fn null_model_binary_matches_r() {
        ...
        for i in 0..5 {
            ...
            assert_close(&format!("resid[{i}]"), model.residuals[(i, 0)], nm.expected_residuals_first5[i], 1e-3);
            assert_close(&format!("W[{i}]"), ww[i], nm.expected_working_weights_first5[i], 1e-3);
        }
    ...
    fn multi_staar_per_trait_from_r() {
        ...
        let ms = match &gt.multi_staar {
            Some(ms) => ms,
            None => { eprintln!("multi_staar section missing, skipping"); return; }
        };

        assert_eq!(ms.n_traits, 2);
        assert!(ms.trait1_staar_o > 0.0 && ms.trait1_staar_o <= 1.0,
            "Trait1 STAAR-O: {}", ms.trait1_staar_o);
        assert!(ms.trait2_staar_o > 0.0 && ms.trait2_staar_o <= 1.0,
            "Trait2 STAAR-O: {}", ms.trait2_staar_o);
        ...
        assert!(combined > 0.0 && combined <= 1.0,
            "Combined MultiSTAAR-O should be valid p: {}", combined);
    }
    ...
    fn scang_params_from_r() {
        ...
        let sc = match &gt.scang {
            Some(sc) => sc,
            None => { eprintln!("scang section missing, skipping"); return; }
        };

        assert!(sc.threshold > 0.0, "SCANG threshold should be positive: {}", sc.threshold);
    ...
    fn meta_staar_scaling_from_r() {
        ...
        let mv = match &gt.meta_staar_validation {
            Some(mv) => mv,
            None => { eprintln!("meta_staar_validation missing, skipping"); return; }
        };
        let s = &gt.staar_continuous;
        ...
        for (i, &u) in s.u.iter().enumerate() {
            ...
            assert_close(
                &format!("U_scaled[{i}]"),
                mv.u_scaled[i], expected, TOL_TIGHT,
            );
        }
        ...
        for (i, k_row) in s.k.iter().enumerate() {
            ...
            assert_close(
                ...
                mv.k_diag_scaled[i], expected, TOL_TIGHT,
            );
        }
    }
}
