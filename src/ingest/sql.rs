...
pub fn generate_script(
    ...
    match analysis.format {
        InputFormat::Tabular => {
            ...
            let _ = writeln!(script, "-- (programmatic: engine.register_csv(\"_ingest_input\", path, {:?}))", delim.char());
        }
        InputFormat::Parquet => {
            let _ = writeln!(script, "-- (programmatic: engine.register_parquet(\"_ingest_input\", path))");
        }
        InputFormat::Vcf => {}
    }
...
fn write_tabular_select(sql: &mut String, analysis: &Analysis) {
    ...
    if let Some(chr_col) = &analysis.chr_col {
        let _ = writeln!(sql, "WHERE upper(regexp_replace(CAST(\"{}\" AS VARCHAR), '^chr', '', 'i')) IN (", chr_col);
