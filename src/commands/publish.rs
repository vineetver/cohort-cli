...
use crate::commands::data::{
    self, FileEntry, PackManifest, MANIFEST_SCHEMA_VERSION, REMOTE_BASE_URL,
};
...
fn publish_pack(
    ...
    // Walk all tables and hash files
    ...
    for table in pack.tables {
        ...
        for entry in walker {
            ...
            let sha256 = data::sha256_file(entry.path())?;
    ...
    if dry_run {
        // Write manifest to stdout for inspection
    ...
    // Upload: files first, manifest last (manifest is the commit point)
    ...
    // Validate: re-fetch manifest from MinIO and check it parses
